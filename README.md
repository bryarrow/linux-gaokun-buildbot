...
- Release: <https://github.com/bryarrow/linux-gaokun-buildbot/releases>
...
```nix
{
  inputs.gaokun3.url = "github:bryarrow/linux-gaokun-buildbot";
...
The module adds its packages through `nixpkgs.overlays`, so a configuration that
sets `nixpkgs.pkgs` directly (which replaces the whole package set) does not get
them. Apply `gaokun3.overlays.default` to that package set yourself, or use the
plain `nixpkgs` module and let the module do it.
